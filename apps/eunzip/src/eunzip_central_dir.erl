...
-spec eocd(ZipHandle, FileSize) -> Result when
    ...
    Result :: {'ok', Eocd :: eunzip:unzip_cd_info()} | {'error', Reason :: atom()}.

eocd(ZipHandle, FileSize) ->
    case eunzip_buffer:new(ZipHandle, FileSize, ?chunk_size) of
        {ok, FileBuffer} ->
...
-spec entries(ZipHandle, FileSize, CdInfo) -> Result when
    ZipHandle :: file:fd(),
    FileSize :: non_neg_integer(),
    CdInfo :: eunzip:unzip_cd_info(),
    Result :: {'ok', Eocd :: eunzip:unzip_cd_info()} | {'error', Reason :: atom()}.

entries(ZipHandle, FileSize, #unzip_cd_info{cd_offset = CdOffset, cd_size = CdSize}) ->
    case eunzip_buffer:new(ZipHandle, FileSize, ?chunk_size, CdOffset + CdSize, CdOffset, forward) of
        ...
        {ok, Chunk, FileBuffer1} ->
            case Chunk of
                <<16#06054b50:32/little, _:48/little, TotalEntries:16/little, CdSize:32/little, CdOffset:32/little, Consumed:16/little>> ->
                    ...
                    {ok, #unzip_cd_info{total_entries = TotalEntries, cd_size = CdSize, cd_offset = CdOffset}, FileBuffer2};
        ...
        {ok, Chunk, _FileBuffer} ->
            case is_zip64(Chunk) of
                true ->
                    ...
                    {ok, #unzip_cd_info{total_entries = TotalEntries, cd_size = CdSize, cd_offset = CdOffset}};
                false ->
                    {error, not_a_zip64}
            end;
        {error, Reason} ->
...
add_entry(#{entries := Entries, range_tree := RangeTree} = Acc, FileName, Entry) ->
    #unzip_cd_entry{local_header_offset = LocalHeaderOffset, compressed_size = CompressedSize} = Entry,
...
to_datetime(<<Year:7, Month:4, Day:5>> = Date, <<Hour:5, Minute:6, Second:5>> = Time) ->
    {{1980 + Year, Month, Day}, {Hour, Minute, Second * 2}}.

need_zip64_extra(#unzip_cd_entry{compressed_size = Cs, uncompressed_size = Ucs, local_header_offset = Offset}) ->
...
merge_zip64_extra(Entry, Extra) ->
    ...
    {Entry1, Zip64Extra1} = case Entry of
        #unzip_cd_entry{uncompressed_size = Ucs} when Ucs =:= 16#ffffffff ->
            <<UncompressedSize:64/little, Rest/binary>> = Zip64Extra,
            {Entry#unzip_cd_entry{uncompressed_size = UncompressedSize}, Rest};
        _ -> {Entry, Zip64Extra}
    end,
    {Entry2, Zip64Extra2} = case Entry1 of
        #unzip_cd_entry{compressed_size = Cs} when Cs =:= 16#ffffffff ->
            <<CompressedSize:64/little, Rest1/binary>> = Zip64Extra1,
            {Entry1#unzip_cd_entry{compressed_size = CompressedSize}, Rest1};
        _ -> {Entry1, Zip64Extra1}
    end,
    {Entry3, _Zip64Extra3} = case Entry2 of
        #unzip_cd_entry{local_header_offset = Offset} when Offset =:= 16#ffffffff ->
            <<LocalHeaderOffset:64/little, Rest2/binary>> = Zip64Extra2,
            {Entry2#unzip_cd_entry{local_header_offset = LocalHeaderOffset}, Rest2};
